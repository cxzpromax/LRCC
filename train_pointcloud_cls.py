...
from ModelNetDataLoader import ModelNetData_10_Loader
import argparse
import numpy as np
import os
import torch
...
def parse_args():
    ...
    parser.add_argument('--data_root', type=str,
                        default='/media/one/系统/chenzhuang/Datasets/modelnet40_normal_resampled/')
...
def test(classifier, loader, num_class=40):
    ...
    for j, data in tqdm(enumerate(loader), total=len(loader)):
        points, target, curvatures = data
        target = target[:, 0]

        curvatures = torch.Tensor(curvatures)
        curvatures = curvatures.cuda()

        points = points.transpose(2, 1)
        points, target = points.cuda(), target.cuda()
        pred = classifier(points, curvatures)
...
def main(args):
    ...
    if args.num_class == 10:
        TRAIN_DATASET = ModelNetData_10_Loader(root=args.data_root, npoint=args.num_point, split='train',
                                               normal_channel=args.normal)
        TEST_DATASET = ModelNetData_10_Loader(root=args.data_root, npoint=args.num_point, split='test',
                                              normal_channel=args.normal)
    else:
        TRAIN_DATASET = ModelNetDataLoader(root=args.data_root, npoint=args.num_point, split='train',
                                           normal_channel=args.normal)
        TEST_DATASET = ModelNetDataLoader(root=args.data_root, npoint=args.num_point, split='test',
                                          normal_channel=args.normal)
    ...
    for epoch in range(start_epoch, args.epoch):  # [0, 200]
        ...
        for batch_id, data in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            points, target, curvatures = data  # points: torch.Size([8, 1024, 4])   column 4: curvature value

            # 随机对点进行删除，变换以及移动等操作
            points = points.data.numpy()
            # points = provider.random_point_dropout(points)
            points = provider.random_scale_point_cloud(points)
            points = provider.shift_point_cloud(points)  # (8, 1024, 3)
            ...
            pred = classifier(points, curvatures)  # batch_size;num_class(8*40)
            loss = criterion(pred, target.long())  # 参数维度不一样，顺序重要
